...
## PAC Instructions (arm64e only)

- **Build setting:** `ENABLE_POINTER_AUTHENTICATION` (defaults to `ENABLE_ENHANCED_SECURITY`)
...
- **Build setting:** `CLANG_ENABLE_STACK_ZERO_INIT` (defaults to `ENABLE_ENHANCED_SECURITY`)
...
- **Build setting:** `ENABLE_C_BOUNDS_SAFETY` (defaults to `NO`, independent of `ENABLE_ENHANCED_SECURITY`)
