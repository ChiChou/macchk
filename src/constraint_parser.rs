...
fn format_fact(entry: &ConstraintEntry) -> String {
    match &entry.value {
        ...
        ConstraintValue::Dict(entries) => {
            let inner: Vec<String> = entries.iter().map(|e| format_fact(e)).collect();
...
fn parse_value(data: &[u8]) -> ConstraintValue {
    ...
    match tag {
        ...
        TAG_UTF8STRING => {
            ConstraintValue::Str(String::from_utf8_lossy(content).to_string())
        }
...
mod tests {
    ...
    const XPCPROXY_SELF: &[u8] = &[
        0x70, 0x26, 0x02, 0x01, 0x01, 0xb0, 0x21, 0x30, 0x09, 0x0c, 0x04, 0x63,
        0x63, 0x61, 0x74, 0x02, 0x01, 0x01, 0x30, 0x09, 0x0c, 0x04, 0x63, 0x6f,
        0x6d, 0x70, 0x02, 0x01, 0x01, 0x30, 0x09, 0x0c, 0x04, 0x76, 0x65, 0x72,
        0x73, 0x02, 0x01, 0x01,
    ];

    // launchd parent constraint: ccat=0, reqs: is-kernel-proc=true
    const LAUNCHD_PARENT: &[u8] = &[
        0x70, 0x45, 0x02, 0x01, 0x01, 0xb0, 0x40, 0x30, 0x09, 0x0c, 0x04, 0x63,
        0x63, 0x61, 0x74, 0x02, 0x01, 0x00, 0x30, 0x09, 0x0c, 0x04, 0x63, 0x6f,
        0x6d, 0x70, 0x02, 0x01, 0x01, 0x30, 0x1d, 0x0c, 0x04, 0x72, 0x65, 0x71,
        0x73, 0xb0, 0x15, 0x30, 0x13, 0x0c, 0x0e, 0x69, 0x73, 0x2d, 0x6b, 0x65,
        0x72, 0x6e, 0x65, 0x6c, 0x2d, 0x70, 0x72, 0x6f, 0x63, 0x01, 0x01, 0xff,
    ...
    const GPUTOOLS_PARENT: &[u8] = &[
        0x70, 0x43, 0x02, 0x01, 0x01, 0xb0, 0x3e, 0x30, 0x09, 0x0c, 0x04, 0x63,
        0x63, 0x61, 0x74, 0x02, 0x01, 0x00, 0x30, 0x09, 0x0c, 0x04, 0x63, 0x6f,
        0x6d, 0x70, 0x02, 0x01, 0x01, 0x30, 0x1b, 0x0c, 0x04, 0x72, 0x65, 0x71,
        0x73, 0xb0, 0x13, 0x30, 0x11, 0x0c, 0x0c, 0x69, 0x73, 0x2d, 0x69, 0x6e,
        0x69, 0x74, 0x2d, 0x70, 0x72, 0x6f, 0x63, 0x01, 0x01, 0xff, 0x30, 0x09,
    ...
    fn ccat_zero_suppressed_in_describe() {
        ...
        let lines = decoded.describe();
        assert!(
            !lines.iter().any(|l| l.starts_with("validation-category: none")),
